<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:70:"E:\phpStudy\WWW\ECShop\public/../application/admin\view\logo\logo.html";i:1536972365;}*/ ?>
...
<table cellpadding="3" cellspacing="1">
  ...
  <tr>
    <td class="first-cell" id="<?php echo $val['ad_id'] ?>">
    <span class="ad_name"><?php echo $val['ad_name'] ?></span>
    </td>
...
<script type='text/javascript'>
  $(document).on('click','.ad_name',function(){
    ...
    var obj = $(this);
    var input = $("<input type='text' class='newname' value='"+ad_name+"' />");
    obj.html(input);
    input.click(function(){return false;});
    input.trigger('oucs');
  })
  $(document).on('blur','.ad_name',function(){
    var ad_name = $('.newname').val();
    var id = $(this).parent('td').attr('id');
    var obj = $(this);
    $.ajax({
      url:"<?php echo url('logo/new_name'); ?>",
      type:'GET',
      ...
      success:function(res){
        // console.log(res);
        if (res.status==0 ) {  
          var input = $("<span class='ad_name' >"+ad_name+"</span>");
          obj.html(input); 
        } else {
          alert(res.message);
        }
